...
const getCachePath = async (fileName: string): Promise<string> => {
  ...
  const { exists } = await getInfoAsync(cacheDirectory + AppConf.CACHE_DIR);
  try {
    if (!exists) {
      await makeDirectoryAsync(cacheDirectory + AppConf.CACHE_DIR, { intermediates: false });
    }
    return cacheDirectory + AppConf.CACHE_DIR + "/" + fileName;
  } catch (err) {
    Log.Echo({ err });
    return "";
  }
...
const PickAvatar = async (success: (file: RNFile) => void, toast?: ReturnType<typeof useToast>) => {
  const res = await fileSystem.PickAssets({
    mediaTypes: "images",
    quality: AppConf.UPLOAD_AVATAR_QUALITY,
    selectionLimit: 1,
    allowsEditing: true
  }, toast);
  if (!res) return;
